...
export default function HeroSection() {
  return (
    <section className="mb-2 mt-4 max-w-7xl mx-auto px-6 lg:px-8 ">
      <div className="bg-[#111] rounded-[2.5rem] overflow-hidden max-h-120 flex flex-col lg:flex-row items-stretch border border-stone-800 shadow-2xl">
        ...
        <div className="w-full lg:w-130 bg-black/50 relative overflow-hidden border-l border-stone-800 group">
          ...
          <div className="absolute inset-0 flex flex-col py-10 overflow-hidden">
            <motion.div
              animate={{ y: ["0%", "-50%"] }}
              transition={{
                duration: 25,
            ...
            >
              ...
              {[...TOP_PRODUCTS, ...TOP_PRODUCTS].map((product, idx) => (
                <div
                  key={idx}
                  className="aspect-4/5 w-full max-h-80  rounded-3xl bg-[#0a0a0a] border border-stone-800 overflow-hidden flex flex-col relative group/item cursor-pointer"
                >
                  <Image
                    src={product.img}
                    alt={product.name}
